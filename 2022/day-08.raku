...
for @forest.kv -> $row, @row {
  for @row.kv -> $index, $height {
    @is-visible[$row][$index] = so ( (@row[0..^$index]).all < $height or (@row[$index^..*]).all < $height );
    @scenic-score[$row][$index] = (@row[0..^$index]).reverse 🌳 $height * @row[$index^..*] 🌳 $height;
  }
}
...
for ([Z] @forest).kv -> $col, @col {
  for @col.kv -> $index, $height {
    @is-visible[$index][$col] ||= so ( (@col[0..^$index]).all < $height or (@col[$index^..*]).all < $height );
    @scenic-score[$index][$col] *= (@col[0..^$index]).reverse 🌳 $height * @col[$index^..*] 🌳 $height;
  }
}
